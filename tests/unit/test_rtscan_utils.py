...
def test_get_dsrc_rt_info(mock_session, mock_dsrc_method_pairs):
    ...
    mock_scalars_2.all.return_value = ['dsrc_a']
    ...
    # Call the function
    dsrc_rt, dsrc_exp_rt, dsrc_pred_rt = get_dsrc_rt_info(uq_comp_id, mock_session, mock_dsrc_method_pairs)

    # Assert that the returned data source strings are in the correct groups
    assert dsrc_rt == ['dsrc_a', 'dsrc_b']
    assert dsrc_exp_rt == ['dsrc_a']
    assert dsrc_pred_rt == ['dsrc_b']


@pytest.mark.parametrize("mock_dsrc, mock_entry_count, expected_query_call_count, mock_pred_bool, expected_pred_bool, "
                         "expected_dupl, expected_pred_to_false",
                         [(['dsrc_a'], 1, 1, 'FALSE', 'FALSE', [], []),  # One data source
                          (['dsrc_a', 'dsrc_b'], 1, 2, 'FALSE', 'FALSE', [], []),  # Two data sources
                          (['dsrc_a'], 2, 0, 'FALSE', 'FALSE', [99], []),  # Duplicate entries for one data source
                          (['dsrc_a'], 1, 1, 'TRUE', 'FALSE', [], [99])  # Predicted flag is incorrect
                          ])
def test_check_experimental_data(mock_session, mock_dsrc_method_pairs, mock_dsrc,
    ...
    uq_comp_id, mock_dsrc_exp_rt, mock_session, mock_dsrc_method_pairs)
...
def test_check_experimental_data_no_exp_rt(mock_session, mock_dsrc_method_pairs):
    """Tests case where no RT is found for an experimental data entry."""
    # Prepare function inputs and return values
    uq_comp_id = 99
    mock_dsrc_exp_rt = ['dsrc_a']
...
def test_check_predicted_flags_pred_data(mock_session, mock_dsrc_method_pairs, predicted_flag, expected_pred_to_true):
    """Tests correction of "predicted" flags."""
    # Prepare function inputs and return values
    uq_comp_id = 99
    mock_dsrc_pred_rt = ['dsrc_a']  # Data sources with predicted RTs
    mock_return = MagicMock()
    mock_return.predicted = predicted_flag
    mock_session.query().filter_by().one_or_none.return_value = mock_return

    # Call the function
    pred_to_true = check_predicted_flags_pred_data(uq_comp_id, mock_dsrc_pred_rt, mock_session, mock_dsrc_method_pairs)
...
def test_predict_bfg_rt(mock_session, mock_models_to_bfg, mock_dsrc_method_pairs):
    ...
    predicted_rt = predict_bfg_rt(99, mock_session, mock_models_to_bfg, mock_check_order, mock_dsrc_rt,
                                  mock_dsrc_method_pairs)

    # Assert
    assert predicted_rt == 7.0
    mock_models_to_bfg['dsrc_c'].assert_called_once_with(15.0)  # Check if the correct model was called
...
def test_predict_rt(mock_session, mock_models_from_bfg, mock_dsrc_method_pairs):
    ...
    mock_models_from_bfg['dsrc_b'].assert_called_once_with(7.0)  # Check if the correct model was called
    mock_session.add.assert_called_once()  # Check if the RT was added to the session


def test_predict_rt_none(mock_session, mock_models_from_bfg, mock_dsrc_method_pairs):
