...
def test_process_thermo():
    ...
    csl_template_path = os.path.join(ROOT_DIR,'tests/integration/fixtures/CSL_v0_process.db')
    csl_copy_path =  os.path.join(ROOT_DIR,'tests/integration/temp/CSL_v0_process.db')
    ...
    with mock.patch('builtins.input', return_value='yes'):  # Mocks user input
        run_process_workflow(inst='lfuby', path_data=data_path, path_csl=csl_copy_path)  # Todo: lfuby_workflow
