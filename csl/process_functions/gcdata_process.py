...
class GcdataProcess(FormatProcess):
    def process(self):
        """Workflow to process GC data files."""
        import logging

        logger = logging.getLogger(__name__)
        logger.info('# Executing workflow to process GC data files')

        # Load defaults and settings
        par_config, adduct_notation = self.load_config()
        par_regex = par_regex_gcdata()
        defaults = defaults_gcdata()
        ...
        logger.info('# End of gcdata workflow')

    def extract_data_regex(self, file, par_regex, file_extra=None):
        """Specific GC data extraction."""
        return extract_data_regex_gcdata(file, par_regex)
